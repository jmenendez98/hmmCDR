...
class hmmCDR:
    def __init__(self, n_iter, tol, merge_distance,
    ...
    def runHMM(self, labelled_methylation_data, transition_matrix, emission_matrix):
        ...
        labelled_methylation_data['HMM_score'] = [score[1]*100 for score in responsibilities]
    ...
    def create_subCDR_df(self, labelled_methylation_data):
        ...
        def merge_CpG_dataframe(dataframe, threshold, merge_distance):
            ...
            merged_bedtool = bedtool.merge(d=merge_distance, c=4, o='mean')  # Use mean score

            # Convert back to DataFrame and round scores
            merged_df = merged_bedtool.to_dataframe(names=['chrom', 'start', 'end', 'score'])
            merged_df['score'] = merged_df['score'].round(5)

            return merged_df
        
        # Step 2: Generate high-confidence CDRs
        CDRs = merge_CpG_dataframe(hmmCDR_scores, self.min_cdr_score, self.merge_distance)
        ...
        if 0.0 < self.min_low_conf_score < self.min_cdr_score:
            low_conf_CDRs = merge_CpG_dataframe(hmmCDR_scores, self.min_low_conf_score, self.merge_distance)

            # Filter low-confidence regions by size
            low_conf_CDRs['size'] = low_conf_CDRs['end'] - low_conf_CDRs['start']
            low_conf_CDRs = low_conf_CDRs[low_conf_CDRs['size'] >= self.min_low_conf_size]
            ...
            low_conf_CDRs = low_conf_bedtool.subtract(cdr_bedtool).to_dataframe(names=['chrom', 'start', 'end', 'score', 'size'])
        ...
        combined_CDRs = pd.concat([CDRs[['chrom', 'start', 'end', 'name', 'score', 'strand']],
    ...
    def hmm_all_chromosomes(self, labelled_methylation_chrom_dict, emission_matrix_chrom_dict=None, transition_matrix_chrom_dict=None):
...
def parse_command_line_arguments():
    ...
    argparser.add_argument('--tol', type=float, default=10, help='Cutoff for model convergence in hmmlearn. (default: 10)')
    ...
    argparser.add_argument('--large_merge_distance', type=int, default=200000, help='Distance to merge subCDRs into a larger CDR annotation. (default: 200000)')
...
def main():
    ...
    if args.e_matrix and args.t_matrix:
        ...
        try:
            ...
            if ':' in emission_input:
                ...
                emission_matrix_chrom_dict[chrom.strip()] = ast.literal_eval(matrix_str.strip())
            else:
            ...
            raise ValueError("Invalid emission matrix format. Use format like: '[[0.008,0.40,0.412,0.18],[0.025,0.9225,0.05,0.0025]]', 'chr1:[[0.008,0.40,0.412,0.18],[0.025,0.9225,0.05,0.0025]]...' or path to a file containing the matrix")
        
        # Check for missing chromosome matrices and print warnings
        for chrom in methylation_chrom_dict:
            if chrom not in emission_matrix_chrom_dict:
                print(f"Warning: No emission matrix given for chromosome {chrom}.\n\tProceeding with default emission matrix for {chrom}: [[0.008,0.40,0.412,0.18],[0.025,0.9225,0.05,0.0025]] (not recommended if emissions are changed!!!)")
                emission_matrix_chrom_dict[chrom] = [[0.008,0.40,0.412,0.18],[0.025,0.9225,0.05,0.0025]]
        ...
        try:
            ...
            if ':' in transition_input:
                ...
                transition_matrix_chrom_dict[chrom.strip()] = ast.literal_eval(matrix_str.strip())
            else:
                # No chromosome specified, use for all chromosomes in methylation_chrom_dict
            ...
            raise ValueError("Invalid transition matrix format. Use format like: '[[0.9999, 0.0001], [0.0025, 0.9975]]', 'chr1:[[0.9999, 0.0001], [0.0025, 0.9975]]...' or path to a file containing the matrix")
        
        # Check for missing chromosome transition matrices and print warnings
        for chrom in methylation_chrom_dict:
            if chrom not in transition_matrix_chrom_dict:
                print(f"Warning: No transition matrix given for chromosome {chrom}.\n\tProceeding with default transition matrix for {chrom}: [[0.9999, 0.0001], [0.0025, 0.9975]]")
                transition_matrix_chrom_dict[chrom] = [[0.9999, 0.0001], [0.0025, 0.9975]]

        # label methylation data if matrices are passed in...
        for chrom in methylation_chrom_dict:
            labelled_methylation_chrom_dict[chrom] = priors.assign_emissions(methylation_chrom_dict[chrom], priors.calculate_emission_thresholds(methylation_chrom_dict[chrom]))
    ...
    combined_subCDRs = combined_subCDRs.sort_values(by=list(combined_subCDRs.columns[:2]))
    combined_subCDRs.to_csv(f'{output_prefix}_sub{args.output_label}.bed', sep='\t', index=False, header=False)

    if args.output_all: # output HMM score bedgraph
        combined_hmm_scores = pd.concat(hmm_scores_chrom_dict.values(), axis=0)
        combined_hmm_scores = combined_hmm_scores.sort_values(by=list(combined_hmm_scores.columns[:2]))
        combined_hmm_scores.to_csv(f'{output_prefix}_scores.bedgraph', sep='\t', index=False, header=False)

    # create final CDR output that merges adjacent subCDRs and reports scoring of over 3
    combined_subCDR_bedtool = pybedtools.BedTool.from_dataframe(combined_subCDRs[combined_subCDRs.iloc[:,3] == f"sub{args.output_label}"])
    combined_big_CDR = combined_subCDR_bedtool.merge(d=args.large_merge_distance, c=2, o='count').to_dataframe(names=['chrom', 'start', 'end', 'count'])
    ...
    combined_big_CDR['itemRgb'] = np.where(
        combined_big_CDR['name'] == f"{args.output_label}", args.main_color, 
        np.where(combined_big_CDR['name'] == f'low_conf_{args.output_label}', args.low_conf_color, ''))
